...
const ReceptionPage = () => {
  const [participantType, setParticipantType] = useState<ParticipantType>('discord')
      ...
      if (participantType === 'discord') {
        ...
        if (data.success) {
            setMembers(prev=> prev.map(member =>
                                      ...
                                      ? {...member, roles: [...member.roles, selectedRole]} : member                                     ))
    ...
    <Container maxWidth="md">
      <Box sx={{ mt: 4 }}>
        <Paper elevation={3} sx={{ p: 4 }}>
          ...
          {message && (
            <Alert severity={message.includes('完了') ? 'success' : 'error'} sx={{ mb: 3 }}>
              {message}
            </Alert>
          )}
